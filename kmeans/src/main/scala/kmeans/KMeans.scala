...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    ...
    var r=points.map(i=>(i,findClosest(i,means))).groupBy(_._2).map(i=>(i._1,i._2.map(j=>j._1)))
    // print(points.map(i=>(i,findClosest(i,means))).groupBy(_._2))
    means.foreach(i=>{
      if (!r.contains(i))
      {
        r=r+(i->List())
  ...
  def update(classified: GenMap[Point, GenSeq[Point]], oldMeans: GenSeq[Point]): GenSeq[Point] = {
    oldMeans.map(i=>findAverage(i,classified(i)))
  }
  ...
  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    if (oldMeans.isEmpty) true
      ...
      val a = oldMeans.head
      val b = newMeans.head
      val sub: Double = math.sqrt((a.x-b.x)*(a.x-b.x)+(a.y-b.y)*(a.y-b.y)+(a.z-b.z)*(a.z-b.z))
      if (sub <= eta) {
  ...
  final def kMeans(points: GenSeq[Point], means: GenSeq[Point], eta: Double): GenSeq[Point] = {
    val re=classify(points,means)
    val oldMeans=means
    val newMeans=update(re,means)
